...
/**
 * §6 — Nightly Reconciliation: compares Shopify source-of-truth with Base44 data.
 * Checks orders (lifecycle_state, total_amount, line counts) and products (price).
 * Logs mismatches to ReconciliationMismatch entity.
 * Optionally auto-corrects if auto_correct=true.
 *
 * Designed to complete within function timeout limits:
 *  - Each section (orders, products) wrapped in try/catch so partial results are still saved
 *  - Throttled API calls with retry logic
 *  - Batched mismatch saves
 */
...
/** Save mismatches in batches of 5, throttled */
async function saveMismatches(base44, mismatches) {
  let saved = 0;
  for (let i = 0; i < mismatches.length; i += 5) {
    const batch = mismatches.slice(i, i + 5);
    await Promise.all(batch.map(m =>
      withRetry(() => base44.asServiceRole.entities.ReconciliationMismatch.create(m)).catch(err => {
        console.error(`[Recon] Failed to save mismatch: ${err.message}`);
      })
    ));
    saved += batch.length;
    if (i + 5 < mismatches.length) await sleep(400);
  }
  return saved;
}
...
Deno.serve(async (req) => {
  ...
  if (scope === 'orders' || scope === 'all') {
    try {
        ...
        for (const order of orders) {
          ...
          if (b44.lifecycle_state !== expectedLifecycle) {
            mismatches.push({
              ...
              detected_at: new Date().toISOString(), auto_corrected: autoCorrect,
            });
            if (autoCorrect) {
              await withRetry(() => base44.asServiceRole.entities.SalesOrder.update(b44.id, { lifecycle_state: expectedLifecycle }));
          ...
          if (Math.abs((b44.total_amount || 0) - shopifyTotal) > 0.01) {
            ...
            if (autoCorrect) {
              await withRetry(() => base44.asServiceRole.entities.SalesOrder.update(b44.id, { total_amount: shopifyTotal }));
      ...
      // Check for ghost orders: paid_unfulfilled in Base44 but not in Shopify
      ...
      console.error(`[Recon] Order section failed: ${orderErr.message}`);
      sectionErrors.push(`Orders: ${orderErr.message}`);
    }
  }

  // ═══════════════════════════════════════════════
  // §6 PRODUCT RECONCILIATION (price drift only)
  // ═══════════════════════════════════════════════
  if (scope === 'products' || scope === 'all') {
    try {
      ...
      // Only load Shopify-sourced products (have external_id) to avoid false "missing" alerts
      ...
      while (url) {
        ...
        for (const product of products) {
          for (const variant of (product.variants || [])) {
            ...
            // Skip existence check for products — raw materials won't be in Shopify
            if (!b44) continue;
            ...
            if (Math.abs((b44.price || 0) - shopifyPrice) > 0.01) {
              ...
              if (autoCorrect) {
                await withRetry(() => base44.asServiceRole.entities.Product.update(b44.id, { price: shopifyPrice }));
      ...
      console.error(`[Recon] Product section failed: ${prodErr.message}`);
      sectionErrors.push(`Products: ${prodErr.message}`);
    }
  }

  // ─── Save mismatches ───
  const saved = await saveMismatches(base44, mismatches);

  await withRetry(() => base44.asServiceRole.entities.AuditLog.create({
    action: 'sync', entity_type: 'ReconciliationMismatch',
    description: `Reconciliation (${scope}): ${mismatches.length} mismatches found, ${corrections.length} auto-corrected${sectionErrors.length ? ` | Errors: ${sectionErrors.join('; ')}` : ''}`,
  })).catch(() => {});

  console.log(`[Recon] Done: ${mismatches.length} mismatches, ${corrections.length} corrections, ${sectionErrors.length} section errors`);
  ...
  try {
    ...
    if (recipients.length === 0) {
      const users = await withRetry(() => base44.asServiceRole.entities.User.filter({ role: 'admin' }));
    ...
    if (recipients.length > 0) {
      ...
      const subject = hasErrors
        ? `⚠️ Shopify Reconciliation — Partial (${sectionErrors.length} section error${sectionErrors.length > 1 ? 's' : ''})`
        : mismatches.length === 0
      ...
      const emailBody = [
        ...
        sectionErrors.length > 0 ? `<p style="color:red;"><strong>Section errors:</strong> ${sectionErrors.join('; ')}</p>` : '',
      ...
      for (const email of recipients) {
        await withRetry(() => base44.asServiceRole.integrations.Core.SendEmail({
          to: email, subject, body: emailBody, from_name: 'Lean Living Sync',
        }));
      }
