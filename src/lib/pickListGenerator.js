...
export async function generatePickList(runId, run) {
  ...
  // 2. Fetch run lines + BOMs first, then filter components to only relevant BOMs
  const [runLines, allBoms, allProducts, allLocations] = await Promise.all([
  ...
  // Fetch BOM components only for the relevant BOMs (avoids 2000-record truncation)
  const allBomIds = allBoms.map(b => b.id);
  const allBomComponents = allBomIds.length > 0
    ? await base44.entities.BomComponent.filter({ bom_id: { $in: allBomIds } }, 'bom_id', 5000)
    : [];
  ...
  for (const line of runLines) {
    ...
    if (!portionBom) {
      missingBoms.push(line.product_name || line.product_sku || line.product_id);
      continue;
    }
  ...
  for (const [pid, agg] of Object.entries(ingredientAgg)) {
    ...
    lines.push({
      ...
      from_location_id: agg.product.default_location_id || null,
  ...
  if (lines.length === 0) {
    if (missingBoms.length > 0) {
      const list = missingBoms.slice(0, 5).join(', ') + (missingBoms.length > 5 ? ` and ${missingBoms.length - 5} more` : '');
      throw new Error(`No portion BOMs (recipes) found for: ${list}. Set up Cook + Portion BOMs for these meals in the Recipes section before generating a pick list.`);
    }
    throw new Error('No pickable ingredients found — check that Cook + Portion BOMs are set up for the meals in this run.');
  }
