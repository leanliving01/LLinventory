...
const ENTITY_TABLE_MAP = {
  ...
  SalesOrderFinancialLine:  'sales_order_financial_lines',
  SalesOrderCost:           'sales_order_costs',
  SalesLineClassificationRule: 'sales_line_classification_rules',
...
export const base44 = {
  ...
  functions: {
    invoke: async (fnName, payload) => {
      ...
      // production so the browser never calls supabase.co directly.
      // In dev the Vite dev-server proxy handles Supabase traffic via /__sb.
      const fnBase = import.meta.env.DEV
        ? `${SUPABASE_URL}/functions/v1`
        : `${window.location.origin}/api/fn`;
