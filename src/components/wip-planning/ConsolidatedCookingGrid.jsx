...
export default function ConsolidatedCookingGrid({
  ...
  const handleReleaseAll = async () => {
    ...
    try {
      ...
      console.error('[ConsolidatedCookingGrid] release all failed:', err);
      toast.error(`Failed to release cooking runs: ${err.message}`);
    } finally {
      setReleasing(false);
    }
  };
  ...
  const handleReleaseSelected = async () => {
    ...
    try {
      ...
      console.error('[ConsolidatedCookingGrid] release selected failed:', err);
      toast.error(`Failed to release selected runs: ${err.message}`);
    } finally {
      setReleasing(false);
    }
  };
  ...
  const handleReleaseSingle = async (row) => {
    ...
    try {
      ...
      console.error('[ConsolidatedCookingGrid] release single failed:', err);
      toast.error(`Failed to release cooking run: ${err.message}`);
    } finally {
    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between bg-muted/50 px-4 py-3 border-b border-border flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {canRelease && (unreleased.length > 0 || draftAdHocRuns.length > 0) && (
            <Button
      ...
      {!isQcConfirmed && canRelease && (unreleased.length > 0 || draftAdHocRuns.length > 0) && (
