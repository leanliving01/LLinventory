...
import { RefreshCw, X, ArrowUp, ArrowDown, Minus, Loader2 } from 'lucide-react';
import { cn } from '@/lib/utils';

/**
 * Recalculate modal — compares existing run lines against latest stock/committed/par
 * and shows a diff of what changed. User can confirm to update the run.
 */
export default function RecalculateRunModal({ runId, existingLines, onConfirm, onCancel }) {
  ...
  const productMap = useMemo(() => {
    const map = {};
    finishedMeals.forEach(p => { map[p.id] = p; });
    return map;
  }, [finishedMeals]);

  // Recalculate recommended qty for each existing line
  const diff = useMemo(() => {
  ...
  }, [existingLines, stockMap, productMap]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-card rounded-2xl shadow-2xl border border-border w-full max-w-3xl max-h-[85vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Updated recommendations based on latest stock & demand
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex items-center gap-6 mb-4 pb-4 border-b border-border">
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase font-semibold">Lines Changed</p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {diff.map(d => (
                    <tr key={d.id} className={cn("hover:bg-muted/30", d.change !== 0 && "bg-amber-50/40")}>
                      <td className="px-3 py-2 text-xs">{d.product_name}</td>
                      ...
                      <td className="px-2 py-2 text-xs text-right tabular-nums">{d.planned_qty}</td>
        ...
        <div className="shrink-0 border-t border-border px-6 py-4 flex items-center justify-between">
          ...
          <Button onClick={handleApply} disabled={applying || isLoading || changedCount === 0} className="gap-2">
            ...
            {applying ? 'Updating...' : `Apply ${changedCount} Change${changedCount !== 1 ? 's' : ''}`}
          </Button>
        </div>
      </div>
    </div>
