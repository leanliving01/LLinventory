...
export default function SalesReport() {
  ...
  const handleExport = () => {
    downloadCSV('sales_report.csv', filtered.map(o => ({
      order_id: o.shopify_order_id, customer: o.customer_name, status: o.status,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <SumCard label="Revenue" value={`R ${totals.revenue.toLocaleString()}`} accent />
          ...
          <tbody className="divide-y divide-border">
            {filtered.length === 0 ? (
              ...
              <tr key={o.id} className="hover:bg-muted/30">
                <td className="px-4 py-2.5 font-medium">{o.shopify_order_id || '—'}</td>
