...
export default function FoodCostReport() {
  ...
  const data = useMemo(() => {
    ...
    const revenue = sales.filter(s => inRange(s.order_date) && !['cancelled', 'refunded'].includes(s.status))
      .reduce((sum, s) => sum + (s.total_amount || 0), 0);

    const rawSpend = pos.filter(p => inRange(p.order_date) && !['draft', 'cancelled'].includes(p.status))
      .reduce((sum, p) => sum + (p.subtotal || 0), 0);

    const wasteValue = wastage.filter(w => inRange(w.wastage_date))
    ...
    const cogs = rawSpend + wasteValue;
    const grossProfit = revenue - cogs;
    const grossMargin = revenue > 0 ? (grossProfit / revenue) * 100 : 0;
    const foodCostPct = revenue > 0 ? (cogs / revenue) * 100 : 0;
    const costPerUnit = totalUnits > 0 ? cogs / totalUnits : 0;

    return { revenue, rawSpend, wasteValue, cogs, grossProfit, grossMargin, foodCostPct, totalUnits, costPerUnit };
  }, [sales, pos, wastage, runs, from, to]);

  const handleExport = () => {
    downloadCSV('food_cost_report.csv', [{
      period: `${format(from, 'dd MMM yyyy')} - ${format(to, 'dd MMM yyyy')}`,
      revenue: data.revenue, raw_spend: data.rawSpend, wastage: data.wasteValue,
      cogs: data.cogs, gross_profit: data.grossProfit, gross_margin_pct: data.grossMargin.toFixed(1),
      food_cost_pct: data.foodCostPct.toFixed(1), units_produced: data.totalUnits,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        ...
        <MetricCard label="Raw Material Spend" value={`R ${Math.round(data.rawSpend).toLocaleString()}`} color="text-blue-700" bg="bg-blue-50 border-blue-200" />
      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-3">
        ...
        <BreakdownRow label="Food Cost %" value={`${data.foodCostPct.toFixed(1)}%`} note="Target < 35%" ok={data.foodCostPct < 35} />
