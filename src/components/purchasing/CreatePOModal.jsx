...
export default function CreatePOModal({ onCreated, onCancel, prefillLines }) {
  ...
  const [lines, setLines] = useState(prefillLines || [{ product_id: '', qty: '', unit_cost: '', uom: '' }]);
  ...
  // Map supplier_sku + supplier_description for search
  const spSearchMap = useMemo(() => {
    const map = {};
    supplierProducts.forEach(sp => {
      map[sp.product_id] = {
        supplierSku: (sp.supplier_sku || '').toLowerCase(),
        supplierDesc: (sp.supplier_description || sp.supplier_description || '').toLowerCase(),
  ...
  const filteredProducts = useMemo(() => {
    let list = products;
    // When a supplier is selected, restrict to their SupplierProduct catalog only
  ...
  const generatePONumber = async () => {
    ...
    try {
      const existing = await base44.entities.PurchaseOrder.list('po_number', 500);
      const maxSeq = existing.reduce((max, po) => {
        if (!(po.po_number || '').startsWith(prefix)) return max;
                      ...
                      <tr key={idx} className={hasVariance ? 'bg-amber-50 dark:bg-amber-950/20' : ''}>
                          ...
                          <Select value={line.product_id} onValueChange={v => {
                            ...
                            <SelectContent>
                              <div className="px-2 pb-2">
                                <Input placeholder={isLoadingSPs ? 'Loading products...' : 'Search...'} value={search} onChange={e => setSearch(e.target.value)} className="h-7 text-xs" disabled={isLoadingSPs} />
                              </div>
                        ...
                        <td className="px-3 py-2">
                          <div className="relative">
                            <Input type="number" value={line.unit_cost} onChange={e => updateLine(idx, 'unit_cost', e.target.value)} placeholder="0.00" className={`h-9 text-sm bg-background${hasVariance ? ' border-amber-400 focus:border-amber-500' : ''}`} min="0" step="0.01" />
                            {hasVariance && (
