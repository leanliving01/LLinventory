...
export default function CreateWriteOffForm({ user, onCreated, onCancel }) {
  ...
  const handleSave = async () => {
    ...
    // Update StockOnHand
    const stockRecords = await base44.entities.StockOnHand.filter({ product_id: product.id }, '-updated_date', 5);
    if (stockRecords.length > 0) {
      const soh = stockRecords[0];
      const newOnHand = Math.max(0, (soh.qty_on_hand || 0) - qtyNum);
