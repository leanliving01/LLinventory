...
export default function NewProductionTable({ title, mealRows, packageTypes, stockValues, onStockChange }) {
            ...
            <tr className="bg-muted/30 border-b border-border">
              {packageTypes.map(pt => (
                <React.Fragment key={pt}>
                  ...
                  <th className="text-right px-1 py-1.5 text-[10px] text-muted-foreground">Add</th>
                </React.Fragment>
              ))}
            </tr>
          </thead>
            ...
            {mealRows.map(row => (
              <tr key={row.mealName} className="hover:bg-muted/30 transition-colors">
                ...
                {packageTypes.map(pt => {
                  ...
                  const addVal = stockValues[sku.id] ?? '';
                  const newTotal = addVal !== '' ? (currentStock || 0) + Number(addVal) : null;
                  return (
                    <React.Fragment key={pt}>
