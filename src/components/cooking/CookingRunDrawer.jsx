...
export default function CookingRunDrawer({ run, onClose, onUpdated }) {
  ...
  const handleComplete = async () => {
    ...
    // Update rolling supplier yield average (supplier scorecard data source)
    if (run.supplier_id && run.raw_product_id) {
      const key = { supplier_id: run.supplier_id, product_id: run.raw_product_id };
      const existing = await base44.entities.SupplierYieldRecord.filter(key, '-created_date', 1);
      if (existing.length > 0) {
        const prev = existing[0];
        const totalRuns = (prev.run_count || 0) + 1;
        const rollingYield = ((prev.avg_yield_pct || 0) * (totalRuns - 1) + yieldPct) / totalRuns;
        const rollingCost = ((prev.avg_cost_per_kg || 0) * (totalRuns - 1) + costPerKg) / totalRuns;
        await base44.entities.SupplierYieldRecord.update(prev.id, {
          avg_yield_pct: Math.round(rollingYield * 10) / 10,
          avg_cost_per_kg: Math.round(rollingCost * 100) / 100,
          run_count: totalRuns,
          last_run_date: run.run_date,
          last_yield_pct: Math.round(yieldPct * 10) / 10,
        });
      } else {
        await base44.entities.SupplierYieldRecord.create({
          supplier_id: run.supplier_id,
          supplier_name: run.supplier_name || '',
          product_id: run.raw_product_id,
          product_name: run.raw_product_name || '',
          avg_yield_pct: Math.round(yieldPct * 10) / 10,
          avg_cost_per_kg: Math.round(costPerKg * 100) / 100,
          run_count: 1,
          last_run_date: run.run_date,
          last_yield_pct: Math.round(yieldPct * 10) / 10,
        });
  ...
  const handleRelease = async () => {
    setUpdating(true);
    try {
      await base44.entities.CookingRun.update(run.id, { status: 'released' });
      toast.success(`${run.run_number} released to kitchen`);
      onUpdated();
    } catch (err) {
        ...
        <div className="sticky bottom-0 bg-card border-t border-border px-6 py-4 shrink-0 flex gap-3 relative z-10">
          ...
          {isDraft && (
            <>
              ...
              <Button onClick={handleRelease} disabled={updating} variant="outline" className="gap-2 h-11 px-6 border-teal-300 text-teal-700 hover:bg-teal-50">
                {updating ? <Loader2 className="w-4 h-4 animate-spin" /> : <CookingPot className="w-4 h-4" />}
                Release
              </Button>
            </>
          )}
