...
import EditSkuCodeDialog from './EditSkuCodeDialog';
...
export default function SKUsTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      <SKUTable title="Goal-Related Meals" items={goalMeals} packageTypes={GOAL_PACKAGE_TYPES} onEditSku={setEditingSku} />
      <SKUTable title="Low Carb Meals" items={lowCarbMeals} packageTypes={LOW_CARB_PACKAGE_TYPES} onEditSku={setEditingSku} />
...
function SKUTable({ title, items, packageTypes, onEditSku }) {
                ...
                {packageTypes.map(pt => {
                  ...
                  return (
                    <td key={pt} className="px-2 py-2.5 text-center border-l border-border">
                      <button
                        className="text-xs font-mono text-muted-foreground hover:text-primary hover:underline cursor-pointer"
