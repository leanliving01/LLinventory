...
export default function ReturnDrawer({ ret, onClose, onUpdated, canProcess }) {
  ...
  const handleMarkReturned = async () => {
    ...
    for (const line of lines) {
      ...
      // Update StockOnHand
      if (locationId) {
        const soh = await base44.entities.StockOnHand.filter({
          product_id: line.product_id,
          location_id: locationId,
        });
        if (soh[0]) {
          const newOnHand = Math.max(0, (soh[0].qty_on_hand || 0) - line.internal_qty_returned);
          await base44.entities.StockOnHand.update(soh[0].id, {
            qty_on_hand: newOnHand,
            qty_available: newOnHand - (soh[0].qty_committed || 0),
