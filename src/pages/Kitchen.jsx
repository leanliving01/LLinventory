...
export default function Kitchen() {
  ...
  // All in-progress runs — kitchen may have concurrent lines
  const { data: runs = [] } = useQuery({
    queryKey: ['active-runs'],
    queryFn: () => base44.entities.ProductionRun.filter({ status: 'in_progress' }, '-run_date', 10),
  });

  // Default to first run; user can switch via run tabs when multiple are active
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {/* Run selector — shown when more than one run is active simultaneously */}
      {runs.length > 1 && (
